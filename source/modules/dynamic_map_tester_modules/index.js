var america = {lat: 60.199713, lng: 24.719375};
var uniqueId = 1;
var markers = [];
var ids = [];
...
function AutocompleteDirectionsHandler(map) {
  var originAutocomplete =
      new google.maps.places.Autocomplete(originInput, {placeIdOnly: true});
  var destinationAutocomplete = new google.maps.places.Autocomplete(
      destinationInput, {placeIdOnly: true});
  var searchAutoComplete =
      new google.maps.places.Autocomplete(search, {placeIdOnly: true});
  ...
  var directionsDisplay = new google.maps.DirectionsRenderer({
    ...
    routeIndex: i,
    polylineOptions:
        {strokeColor: '#208000', strokeWeight: 10, strokeOpacity: .95},
  ...
  var onChangeHandler = function() {
    calculateAndDisplayRoute(
        directionsService, directionsDisplay, markers, stepDisplay, map);
  };
  ...
  var request = {
    ...
    drivingOptions: {
      departureTime: new Date(
          Date.now() + 3500000),  // for the time N milliseconds from now.
    ...
    if (status === 'OK') {
      ...
      for (i = 0; i < routes.length; i++) {
        ...
        new google.maps.DirectionsRenderer({
          ...
          routeIndex: i + 1,
          polylineOptions:
              {strokeColor: colors[i], strokeWeight: 8, strokeOpacity: .8},
      ...
      console.log(
          'Direction Service Request STATUS : ' + status + result.routes)
...
function showSteps(result, markers, stepDisplay, map) {
  ...
  for (var i = 0; i < myRoute.steps.length; i++) {
    marker = markers[i] = markers[i] ||
        new google.maps.Marker(
            {map: map, position: myRoute.steps[i].start_location});
...
function initMap() {
  ...
  */
  ...
  document.getElementById('submit').addEventListener('click', function() {
    ...
    console.log(
        'geoCoder submitted user\'s address input of : ' + address.value);
  });
...
function clickedMarker(location, map) {
  clearConsole();
  console.log(
      'addMarker function completed at point: ' + location + ' ' +
      '.');
  ...
  console.log(
      'InfoWindow was opened by contentLoad function for clicked-on marker: ' +
      marker.id);
...
function contentClickedMarker(marker) {
  var contentTestString = '<div id = "contentTestString">' +
      '<div id="sideNotice">' +
      '<h1 id="heading" class="heading">' +
      'Click the button below to completely remove this marker.</h1>' +
      '<br>' +
      '<p id = "text" >You may close this window by simply cliking the X on the top right corner!.</p>' +
      '<br>' +
      '<p id="text">Marker Location:' + marker.getPosition() + '<br>' +
      'Marker\'s ID is :' + marker.id + '<br>' +
      '</p>' +
      '<form action = "">' +
      '<fieldset>' +
      '<input type = "button" id="buttonRemover" value="Remove This Marker" onclick ="deleteMarkers(' +
      marker.id + ')"/>' +
      '</div>' +
      '</div>' +
      '';
...
function geoAddressMarkerLoad(marker, results) {
  var contentTestString2 = '<div id="littleWindow">' +
      '<div id="contentTestString2">' +
      '<div id="sideNotice">' +
      '<br>' +
      '<p id="location">Marker Location:' +
      '</p>' +
      '<p id="latLong">' + marker.getPosition() + '</p>' +
      'Marker\'s ID is : ' + marker.id + '<br>' +
      'Address type(s) : ' + results[0].types + '<br>' +
      'Marker\'s formatted address is : ' + results[0].formatted_address +
      '<br>' +
      'Location Type : ' + results[0].geometry.location_type + '<br>' +
      '</div>' +
      '<br>' +
      '<form action = "">' +
      '<fieldset>' +
      '<input type = "button" id="buttonRemover" value="Remove Marker" onclick ="deleteMarkers(' +
      marker.id + ')"/>' +
      '<br>' +
...
function findAddress(map, panorama) {
  ...
  var locale = latLng1;
  console.log(locale);
  geocoder.geocode({'location': locale}, function statusTest(results, status) {
    ...
    if (status == 'OK') {
      ...
      if (results[0]) {
        console.log(
            'findAddress function location result is: ' +
...
function findAddressMarkerLoad(marker, results) {
  var contentTestString3 = '<div id="returnFind">' +
