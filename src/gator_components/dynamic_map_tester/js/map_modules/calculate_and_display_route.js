...
export default function calculateAndDisplayRoute(directionsService, directionsDisplay, markers, stepDisplay, map) {
	...
	directionsService.route(request, function(result, status) {
		if (status === 'OK') {
			...
			let showSteps = require('./map_modules/show_steps.js');
			showSteps(result, markers, stepDisplay, map);

			let route = result.routes[0];
			
			let summaryPanel = document.getElementById('warnings-panel');
